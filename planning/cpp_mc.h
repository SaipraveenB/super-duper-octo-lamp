...
std::vector<int> GetActionValueEstimate(const std::vector<std::vector<float>>& rewards,
    ...
    struct Environment {
        ...
        Environment(std::pair<int,int> env_shape, std::pair<int,int> kern_shape,
                    const std::pair<int,int> start_state,const std::pair<int,int> end_state) {
        ...
        // Define env play() here.
        std::tuple<std::pair<int,int>,float,bool> play(int action) {
            // TODO(SaiPraveenB) : Can you fill in the delta visibility calculations and visibility updating?
            int delta_vis;
            // Calculate delta(visibility)
            // Update visibility

            // Calculate psuedo rewards, actual reward
